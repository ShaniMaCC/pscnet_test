// c onst userDataUrl ='http://10.107.7.26:8080/SecuredLoanWeb/base_data?kind=base_data&custID=4010-091001';
...
//   return user;
// }
// getUserDataJSON().then(user => {
//   userData.push(user); // fetched movies
//   console.log(userData);
// });
let userData = [];
function ajaxCallJsonp1() {
  $.ajax({
    url: "http://10.107.7.26:8080/SecuredLoanWeb/base_data",
    type: "post",
    async: false,
    dataType: "json",
...
function ajaxCallJsonp2() {
  $.ajax({
    url: "http://10.107.7.26:8080/SecuredLoanWeb/base_data",
    type: "post",
    async: false,
    dataType: "json",
    data: { kind: "acc_data", custID: "4010-091000" },
    success: function (rtn) {
      userAccData.push(rtn);
      // console.log(' success ' + JSON.stringify(rtn));
    },
    error: function (rtn) {
      console.log("errors" + JSON.stringify(rtn));
...
// let userStockData = [];
// function ajaxCallJsonp3() {
//   $.ajax({
//     url: "http://10.107.7.26:8080/SecuredLoanWeb/base_data",
//     type: "post",
//     async: false,
//     dataType: "json",
//     data: { kind: "S50", custID: "4010-091000" },
//     success: function (rtn) {
//       userStockData.push(rtn);
//       // console.log(' success ' + JSON.stringify(rtn));
//     },
//     error: function (rtn) {
//       console.log("errors" + JSON.stringify(rtn));
...
// ajaxCallJsonp3();
// function ajaxCallJsonp1() {
//   let custID = $("inpuut[name=user-account]").val();
//   $("#msg").html("新增中...");
...
//     url: "http://10.107.7.26:8080/SecuredLoanWeb/base_data",
//     type: "post",
//     data: { kind: "base_data", custID: userAcc.value },
//     dataType: "json",
//     success: function (rtn) {
//       userDatax.push(rtn);
//       console.log(rtn);
//       sessionStorage.setItem("userData", JSON.stringify(userDatax));
...
function userLogin() {
  // let data = { kind: 'base_data',custID: userAcc.value };
...
if (window.location.pathname == "/loan.html") {
  ...
  user_a1.value = userData[user_index].a1
    .split(".")[0]
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  user_a2.value = userData[user_index].a2
    .split(".")[0]
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  user_a3.value = userData[user_index].a3
    .split(".")[0]
    .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  ...
  if (searchTime) {
    searchTime.value = `${today.getFullYear()}/${
      today.getMonth() + 1
...
if (
  window.location.pathname == "/acc_info.html" ||
  window.location.pathname == "/check_overloan.html"
) {
...
if (window.location.pathname == "/acc_overloan.html") {
  ...
  console.log(user_stock_arr.length); //14
  console.log(stock_len); //14
  let stocklist_str = "";

  for (let i = 0; i < stock_len; i++) {
  // for (let us_info in user_stock_arr) {
    let stock_index = stockData.findIndex(
      (x) => x.stockID == user_stock_arr[i].stockID
    );
    ...
    let stocklist_content = `
      <tr>
      <td name="stock-id">${user_stock_arr[i].stockID}</td>
      <td>${stockData[stock_index].stockName}</td>
      <td name="stock-qty">${user_stock_arr[i].ableLoanQty / 1000}</td>
  ...
  qtys.forEach((e) => {
    e.addEventListener("change", () => {
      ...
      if (
        parseInt(user_stockNum) < parseInt(qty_arr[qtynodes_arr.indexOf(e)])
      ) {
...
if (
  window.location.pathname == "/check_overloan.html" ||
  window.location.pathname == "/acc_overloan.html"
) {
  ...
  let empty_arr = user_loan.productQty.filter(
    (x) => x === "" || x == "0" || x === 0 || x == null
  );
  ...
  for (let i = 0; i < user_loan.productQty.length; i++) {
    if (
      user_loan.productQty[i] === "" ||
      user_loan.productQty[i] === "0" ||
      user_loan.productQty[i] === 0 ||
      user_loan.productQty[i] === null
    ) {
